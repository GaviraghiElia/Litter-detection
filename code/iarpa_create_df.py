    ...
    if os.path.isdir(suite_path):
        ...
        if os.path.isfile(manifest_file):
            ...
            if language not in ['c', 'c++', 'cplusplus']:
                print(f"language not supported: {language} into file {manifest_file}")
                continue

            for run in sarif_data['runs']:
                for result in run['results']:
                    cwe_class = result['ruleId']
                    cwe_classes = cwe_class
                    ...
                    if 'locations' in result and result['locations']:
                        ...
                        try:
                            ...
                            print(f"Errore durante la lettura del file {source_file}: {e}")
...
df.to_csv('iarpa.csv', index=False)
print(df.head())
print(df.shape)
print(df.iloc[1])